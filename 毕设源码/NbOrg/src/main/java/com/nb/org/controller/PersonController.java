...
public class PersonController {
	...
	public String PersonDtail(@PathVariable(value="id") int id ,@RequestParam("dep") String depname,HttpSession session,Model model){
		Department department = (Department) session.getAttribute("dep");
		...
		Department dep = null;
		dep = departmentService.selectDepByFullName(depname);
	...
	public String PersonEdit(@PathVariable(value="id") int id ,@RequestParam("dep") String depname,HttpSession session,Model model){
		//Person user= (Person) session.getAttribute("user");
		Department department=(Department) session.getAttribute("dep");
		List<Department> list=new ArrayList<Department>();
		list.add(department);
	 ...
	 * @throws
	 */
	private List<PersonDto> PersonToPersonDto(Department department,List<Person> list){
	 ...
	 * @throws
	 */
	private List<PersonDto> SearchPersonToPersonDto(Department department,Department dep,List<Person> list){
		 List<PersonDto> reslist=new ArrayList<PersonDto>();
		 int samePersonId=0;
		 for(Person person :list){
			 samePersonId++;
