        ...
        <div class="col-md-8">
            ...
            <nav class="text-center">
                ...
                <c:choose>
                  ...
                  <c:otherwise>
                   <li><a href="department/list?pageNo_mine=1">首页</a></li>
                  <li><a href="department/list?pageNo_mine=${pageIndex-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
                  </c:otherwise>
                </c:choose>  
                <c:if test="${pageTotal<6 }">
                <%for(int i=1;i<=pageTotal;i++) {
                if(pageIndex==i) {%>
                <li class="active"><a href="department/list?pageNo_mine=<%=i%>"><%=i%><span class="sr-only">(current)</span></a></li>
                <%} else {%>
                    <li><a href="department/list?pageNo_mine=<%=i%>"><%=i%></a></li>
                   <% }%>
                    <% }%>
                      </c:if>
                    <c:if test="${(pageTotal>=6)&&(pageIndex>2)&&(pageIndex<pageTotal-2) }">
                <li><a href="department/list?pageNo_mine=${pageIndex-2 }">${pageIndex-2 }</a></li>
                <li><a href="department/list?pageNo_mine=${pageIndex-1 }">${pageIndex-1 }</a></li>
               <li class="active"><a href="department/list?pageNo_mine=${pageIndex }">${pageIndex }<span class="sr-only">(current)</span></a></li>
               <li><a href="department/list?pageNo_mine=${pageIndex+1 }">${pageIndex+1 }</a></li>
               <li><a href="department/list?pageNo_mine=${pageIndex+2 }">${pageIndex+2 }</a></li>
                </c:if>
              ...
              <%for(int i=pageTotal-4;i<=pageTotal;i++) {
                ...
                <%} else {%>
                  ...
                  <c:otherwise>
                  <li><a href="department/list?pageNo_mine=${pageIndex+1 }" aria-label="Previous"><span aria-hidden="true">&raquo;</span></a></li>
                  <li><a href="department/list?pageNo_mine=${pageTotal }">尾页</a></li>
                  </c:otherwise>
                </c:choose>  
                </ul>
            </nav>
